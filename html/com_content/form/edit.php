...
$isSelfPoster = in_array(16,$groups);
$isStaff = !$isSelfPoster;
...
<form action="<?php echo JRoute::_('index.php?option=com_content&a_id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="adminForm" class="form-validate">
	<fieldset>
		...
		<span style="color:#800000"> 注：多个关键词请使用英文字符逗号( , )隔开</span>
		</div>
<!--categories selector   -->	
		<div class="formelm" style="margin-bottom: 40px;">
		...
		<div class="formelm">
		 ...
		 <?php if ($isSelfPoster){ 
			 ...
			 echo "<span style='color:#800000'>请注意控制您的广告字数不能超过160字。如果超过，系统会自动屏蔽显示，谢谢您的合作.</span>";		     
		 }?>
...
<?php if ($isStaff) {	?>
	...
	<fieldset>
		<legend><?php echo JText::_('COM_CONTENT_IMAGES_AND_URLS'); ?></legend>
		    <div style="float:left;border:2px dotted #ddd;">
			<div class="formelm">
		  ...
		  <div style="float:right; border:2px dotted #ddd;">
			<div class="formelm">
 ...
 <?php } ?>
	...
	<?php if ($this->item->params->get('access-change')): ?>
      ...
      <?php if ($isStaff) {	?>
		...
		<?php echo $this->form->getInput('featured'); ?><br/>
		<span style="color:#800000;font-size:11px;">只适用于新闻模块, 如果要把这条新闻设为置顶，请必须把当前置顶显示的那一条新闻，设为“否”,才能正确显示！</span>
		</div>
      <?php } ?>
	...
	<?php endif; ?>
		...
		<?php if (is_null($this->item->id)):?>
			<div class="form-note">
			<p><?php //echo JText::_('COM_CONTENT_ORDERING');    
...
<!--
 	<label class="option" for="edit-sticky">
        <input id="edit-sticky" type="checkbox" value="1" name="sticky">
                Sticky Article
    </label>   	
...
-->
		<input type="hidden" name="task" value="" required="" />
